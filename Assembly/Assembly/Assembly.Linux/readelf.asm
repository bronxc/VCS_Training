...
section .data
	...
	; field: collum
	msgColShField db "  [Nr]  Name", 09h, 09h, "Type", 09h, 09h, "Addr", 09h, "Off", 09h, "Size", 09h, "ES  Flg  Lk  Inf  Al"
	len_msgColShField equ $-msgColShField
	
	; sh_type: sh_type_table
	sh_type_table	db "NULL         ", 0
					db "PROGBITS     ", 0
					db "SYMTAB       ", 0
					db "STRTAB       ", 0
					db "RELA         ", 0
	...
	len_sh_type equ ($-sh_type_table)/20
	
	; sh_flags: sh_flags_table
	sh_flags_table	db "   ", 0
					db "W  ", 0
					db "A  ", 0
					db "WA ", 0
	...
	len_sh_flags equ ($-sh_flags_table)/11

	
	; ----------------------------------------------
	; program header
	; ----------------------------------------------
	msgProgramHeader db "Progam Header:", 0
	len_msgProgamHeader equ $-msgProgramHeader
	
	msgElfFileType db "Elf file type is ", 0
	len_msgElfFileType equ $-msgElfFileType
	
	msgEntryPoint db "Entry point ", 0
	len_msgEntryPoint equ $-msgEntryPoint
	
	msgColPhField db "  Type", 09h, 09h, "Offset   ", 09h, "VirAddress", 09h, "PhysAddr", 09h, "FileSiz ", 09h, "MemSiz  ", 09h, "Flg  Align", 0
	len_msgColPhField equ $-msgColPhField
	
	; ph_type: ph_type_table
	ph_type_table	db "NULL   ", 0
					db "LOAD   ", 0
					db "DYNAMIC", 0
					db "INTERP ", 0
					db "NOTE   ", 0
	...
	len_ph_type equ ($-ph_type_table)/9
	
	; ph_plags: ph_flags_table
	ph_flags_table	db "   ", 0
					db "E  ", 0
					db "W  ", 0
					db "WE ", 0
	...
	len_ph_flags equ ($-ph_flags_table)/8
...
readelf_l:
	...
	@loop_felf_phdr:
		cmp edi, dword [count]
		je @done_loop_felf_phdr
		
		call printEnter
		call printSpaceX2
		; call print field (in that proc having get)
		call print_elf_phdr_type
		call print_elf_phdr_offset
		call print_elf_phdr_vaddr
		call print_elf_phdr_paddr
		call print_elf_phdr_filesz
		call print_elf_phdr_memsz
		call print_elf_phdr_flags
		call print_elf_phdr_align
...
; ----------------------------------------------
; print p_offset
print_elf_phdr_offset:
	call print0x	
	call get_program_header_off
	
	cmp byte [OSclass], 1
	jne @felf_phdr_offset64
	
	@felf_phdr_offset32:
		add eax, 0x04
		mov ebx, eax
		add ebx, 3
		jmp @felf_phdr_offset
	...
	;call printFieldData
	;jmp @print_elf_phdr_offset
	
	cmp byte [OSclass], 1
	je @print_elf_phdr_offset
	add ebx, 4
...
; ----------------------------------------------
; print p_vaddr
print_elf_phdr_vaddr:	
	call print0x
	call get_program_header_off
	
	cmp byte [OSclass], 1
	jne @felf_phdr_vaddr64
...
; ----------------------------------------------
; print p_paddr
print_elf_phdr_paddr:	
	call print0x
	call get_program_header_off
	
	cmp byte [OSclass], 1
	jne @felf_phdr_paddr64
...
; ----------------------------------------------
; print p_filesz
print_elf_phdr_filesz:
	call print0x	
	call get_program_header_off
	
	cmp byte [OSclass], 1
	jne @felf_phdr_filesz64
	
	@felf_phdr_filesz32:
		add eax, 0x10
		mov ebx, eax
		add ebx, 3
		jmp @felf_phdr_filesz
...
; ----------------------------------------------
; print p_memsz
print_elf_phdr_memsz:
	call print0x	
	call get_program_header_off
	
	cmp byte [OSclass], 1
	jne @felf_phdr_memsz64
...
; ----------------------------------------------
; print p_flags
print_elf_phdr_flags:
	call get_program_header_off
	
	cmp byte [OSclass], 1
	jne @felf_phdr_flags64
	...
	; init input to proc
	; offset code
	mov ebx, len_ph_flags
	mul ebx
	
	mov ecx, ph_flags_table
	add ecx, eax
	mov edx, len_ph_flags
...
; ----------------------------------------------
; print p_align
print_elf_phdr_align:	
	call get_program_header_off
	
	cmp byte [OSclass], 1
	jne @felf_phdr_align64
...
get_program_header_off:
	; size of program header
	xor eax, eax
	mov ax, word [sElfHeader + e_phentsize]
	; offset of the program header
	mov ebx, dword [count]
	; get program header offset (at the begin of all)
	mov ecx, dword [phOffset]
	; get begin of section
	mul ebx
	add ecx, eax
	mov eax, data
	add eax, ecx										; begin of the program header offset in data
	
	ret


; ----------------------------------------------
; print Program Section Message
print_ph_msg:
	call printEnter
	mov ecx, msgElfFileType
	mov edx, len_msgElfFileType
	call printMsg
	...
	mov ecx, msgEntryPoint
	mov edx, len_msgEntryPoint
	call printMsg
	...
	; program header:
	mov ecx, msgProgramHeader
	mov edx, len_msgProgamHeader
	call printMsg
	call printEnter
	
	mov ecx, msgColPhField
	mov edx, len_msgColPhField
	call printMsg
...
readelf_S:
	...
	mov ecx, msgSectionHeader
	mov edx, len_msgSectionHeader
	call printMsg
	call printEnter
	
	mov ecx, msgColShField
	mov edx, len_msgColShField
	call printMsg
	
	; get string table offset
	call getStrTable
	
	; get number of section header
	xor eax, eax
	mov ax, word [sElfHeader + e_shnum]
	; store to count to loop
	mov dword [count], 0								; begin of loop
	mov edi, eax										; end of loop
	
	
	@loop_felf_shdr:
		cmp edi, dword [count]
		jle @done_loop_felf_shdr
		
		call printEnter
		call printSpaceX2
		; call print field (in that proc having get)
		call print_elf_shdr_Nr
		call print_elf_shdr_name
		call print_elf_shdr_type
		call print_elf_shdr_addr
		call print_elf_shdr_offset
		call print_elf_shdr_size
		call print_elf_shdr_entsize
		call print_elf_shdr_flags
		call print_elf_shdr_link
		call print_elf_shdr_info
		call print_elf_shdr_addralign
	...
	call printEnter
	call printEnter
	mov ecx, msgKeysToFlags
	mov edx, len_msgKeysToFlags
	call printMsg
	call printEnter
	ret
...
print_elf_shdr_name:
	...
	add esi, dword [stOffset]
	
	@loop_print_cname:
		cmp byte [esi], 0
		je @done_loop_print_cname
		...
		inc esi
		jmp @loop_print_cname
		
	@done_loop_print_cname:
	call printSpaceX2
	call printSpaceX2
	call printSpaceX2
	call printSpaceX2
...
print_elf_shdr_flags:	
	...
	mov eax, dword [field]
	cmp byte [OSclass], 1
	je @print_elf_shdr_flags
	mov eax, dword [field + 4]
	...
	; offset code
	cmp eax, 10
	jle _print_elf_shdr_flags
	mov eax, 11
	
	_print_elf_shdr_flags:
	mov ebx, len_sh_flags
	mul ebx
	
	mov ecx, sh_flags_table
	add ecx, eax
	mov edx, len_sh_flags
...
; ----------------------------------------------
; print sh_offset
print_elf_shdr_offset:	
	call get_section_header_off
	
	cmp byte [OSclass], 1
	jne @felf_shdr_offset64
	@felf_shdr_offset32:
		add eax, 0x10
		mov ebx, eax
		add ebx, 3
		jmp @felf_shdr_offset
...
; ----------------------------------------------
; print sh_addralign
print_elf_shdr_addralign:	
	call get_section_header_off
	
	cmp byte [OSclass], 1
	jne @felf_shdr_addralign64
	@felf_shdr_addralign32:
		add eax, 0x20
		mov ebx, eax
		add ebx, 3
		jmp @felf_shdr_addralign
	
	@felf_shdr_addralign64:
		add eax, 0x30
		mov ebx, eax
		add ebx, 7
...
; ----------------------------------------------
; print sh_entsize
print_elf_shdr_entsize:	
	call get_section_header_off
	
	cmp byte [OSclass], 1
	jne @felf_shdr_entsize64
	@felf_shdr_entsize32:
		add eax, 0x24
		mov ebx, eax
		add ebx, 3
		jmp @felf_shdr_entsize
...
get_section_header_off:
	; size of section header
	xor eax, eax
	mov ax, word [sElfHeader + e_shentsize]
	; offset of the section header
	mov ebx, dword [count]
	; get section header offset (at the begin of all)
	mov ecx, dword [shOffset]
	; get begin of section
	mul ebx
	add ecx, eax
	mov eax, data
	add eax, ecx										; begin of the section header offset in data
...
; ----------------------------------------------
; print left square bracket into file
printLSBracket:

	; input: cLSBracket (defaut)
	mov eax, 4
	mov ebx, 1
	mov ecx, cLSBracket
	mov edx, len_cBracket
	int 0x80
	
	ret	
	
; ----------------------------------------------
; print right square bracket into file
printRSBracket:

	; input: cRSBracket (defaut)
	mov eax, 4
	mov ebx, 1
	mov ecx, cRSBracket
	mov edx, len_cBracket
...
; ----------------------------------------------
; print address hex	withou delete rezo
printAddrWith0:
	; input. eax: point to field; ebx: point to the end of field
	mov ebp, eax
...
; ----------------------------------------------
; get string table offset - .shstrtab section	
getStrTable:
	...
	; get section .shstrtab header index (start from 0)
	xor ebx, ebx
	mov bx, word [sElfHeader + e_shstrndx]
	...
	; get string table offset - .shstrtab section
	mov eax, data												; 0x00 of data
	add eax, ecx												; .shstrtab section header offset

	
	add eax, 0x10												; 32bit sh_offset offset
	...
	cmp byte [OSclass], 1
	je @getStrTable
	add eax, 8													; 64bit sh_offset offset (0x18)
	add ebx, 12
	...
	; move the value => store to stOffset
	mov eax, dword [strTableOff]
	
	cmp byte [OSclass], 1
	je @done_getStrTable
	mov eax, dword [strTableOff + 4]
	
	@done_getStrTable:
		mov ecx, data
		add eax, ecx
		mov dword [stOffset], eax
		ret